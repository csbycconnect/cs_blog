/**
 * PixelBlast.jsx
 * Converted from TypeScript to plain JSX for the ByteBoard project.
 * Original concept: github.com/zavalit/bayer-dithering-webgl-demo
 * Adapted: TypeScript interfaces/types removed; import paths corrected.
 */
...
/** Creates a canvas-based touch-trail texture used for the liquid warp effect. */
...
void main(){
  ...
  if(uEnableRipples==1){
    for(int i=0;i<MAX_CLICKS;++i){
      ...
      if(pos.x<0.)continue;
      float cpxSize=8.*pixelSize;
      vec2 cuv=(((pos-uResolution*.5-cpxSize*.5)/(uResolution)))*vec2(aspectRatio,1.);
      float t=max(uTime-uClickTimes[i],0.);
      float r=distance(uv,cuv);
      float waveR=speed2*t;
...
export default function PixelBlast({
    ...
    useEffect(() => {
        ...
        if (mustReinit) {
            ...
            if (t.liquidEffect) {
                t.liquidEffect.uniforms?.get?.('uStrength') && (t.liquidEffect.uniforms.get('uStrength').value = liquidStrength);
                t.liquidEffect.uniforms?.get?.('uFreq') && (t.liquidEffect.uniforms.get('uFreq').value = liquidWobbleSpeed);
            }
